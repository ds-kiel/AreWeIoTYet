//
// Original Copyright (c) 2010-2023 Antmicro under the MIT License
// https://github.com/renode/renode-infrastructure/blob/6a0ba28d7530e391971f4443bf98976c7074284f/src/Emulator/Peripherals/Peripherals/UART/NRF52840_UART.cs
//  Modified to work with RIOT OS
//
using System;
...
{
    public class NRF52840_UART_MODIFIED : UARTBase, IDoubleWordPeripheral, IKnownSize
    {
        public NRF52840_UART_MODIFIED(Machine machine, bool easyDMA = false) : base(machine)
        {
        ...
        protected override void CharWritten()
        {
            ...
            lock(interruptManager)
            {
                ...
                if(easyDMA)
                {
                    ...
                    this.Log(LogLevel.Noisy, "Transfering 0x{0:X} to 0x{1:X} (of 0x{2:X}/0x{3:X})", character, currentRxPointer, currentRxValue, rxMaximumCount.Value);
                    this.Machine.SystemBus.WriteByte(currentRxPointer, character);
                    currentRxValue++;
                    currentRxPointer++;
                    if(currentRxValue == rxMaximumCount.Value)
                    {
                        rxAmount.Value = currentRxValue;
                        interruptManager.SetInterrupt(Interrupts.EndReceive);
                        // ADDED CONTENT!
                        if(shortcutStartrx.Value) {
                            StartRx();
                        }else if(shortcutStoprx.Value) {
                            StopRx();
                        }
                    }
                }
        ...
        private Dictionary<long, DoubleWordRegister> DefineRegisters()
        {
            ...
            else
            {
                ...
                dict.Add((long)Registers.EndTx, GetEventRegister(Interrupts.EndTransmit, "EVENTS_ENDTX")); // Added content this seems like a typo
                ...
                // ADDED CONTENT!!
                dict.Add((long)Registers.Shortcuts, new DoubleWordRegister(this)
                    .WithFlag(5, out shortcutStartrx, name: "ENDRX_STARTRX")
                    .WithFlag(6, out shortcutStoprx, name: "ENDRX_STOPRX")
                    );
        ...
        // Added content!!
        private uint currentRxValue; // RXD.AMOUNT register should be valid following the ENDRX event
        private IFlagRegisterField shortcutStartrx;
        private IFlagRegisterField shortcutStoprx;
